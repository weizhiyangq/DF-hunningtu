...
oof_lgb_median_mad_train=pd.read_csv('oof_lgb_median_mad_train.csv',sep='\t')
oof_lgb_median_mad_test=pd.read_csv('oof_lgb_median_mad_test.csv',sep='\t')

oof_lgb_median_mad_train_rate001=pd.read_csv('oof_lgb_median_mad_train_rate0.01.csv',sep='\t')
oof_lgb_median_mad_test_rate001=pd.read_csv('oof_lgb_median_mad_test_rate0.01.csv',sep='\t')
...
oof_lgb_median_mad=oof_lgb_median_mad_train['lgb_fea'].tolist()
oof_lgb_median_mad_rate_001=oof_lgb_median_mad_train_rate001['lgb_fea'].tolist()
...
predictions_lgb_median_mad=oof_lgb_median_mad_test['lgb_fea'].tolist()
predictions_lgb_median_mad_rate_001=oof_lgb_median_mad_test_rate001['lgb_fea'].tolist()
...
print('shape:\n')
print(len(oof_lgb))
print(len(oof_lgb_median_mad))
# 将lgb和xgb的结果进行stacking
train_stack = np.vstack([oof_lgb,oof_lgb_median_mad_rate_001,oof_svc]).transpose()
test_stack = np.vstack([predictions_lgb, predictions_lgb_median_mad_rate_001,predictions_svc]).transpose()
...
print(submit)
submit.to_csv(r'layer1_lgb_lgbmadmedian_rate001.csv',index=None)
